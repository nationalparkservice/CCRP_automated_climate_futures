...
## Combine new and old Future_Means for scatterplot
Future_Means_old <- read.csv(paste0(DataDir, SiteID, "_Future_Means.csv")) %>%
  mutate(CF = NULL) %>% left_join(WB_GCMs,by="GCM")


# Need to revise if selecting w/in quadrants
############################################################### Begin output plots ############################################
...
dualscatter = ggplot(Future_Means_old, aes(DeltaTavg, DeltaPr*365, xmin=Tavg25, xmax=Tavg75, ymin=Pr25*365, ymax=Pr75*365))
