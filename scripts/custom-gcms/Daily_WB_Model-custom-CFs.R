...
wb_sites <- read.csv(paste0(OutDir, "WB_site_parameters.csv"))
######################################################### END CLIMATE INPUTS ####################################################################
...
CF3.WBplot <- WBplot(scenario=CFs[3],cols=colors2[3])

WBgrid <- ggarrange(Hist.WBplot, CF1.WBplot, CF2.WBplot, CF3.WBplot, ncol = 1, nrow = 4,common.legend = T)
...
WBgrid <- ggarrange(Hist.WBplot, CF1.WBplot, CF2.WBplot, CF3.WBplot, ncol = 4, nrow = 1,common.legend = T)
annotate_figure(WBgrid, left = textGrob("Water (in)", rot = 90, vjust = 1, gp = gpar(cex = 1.3)),
                bottom = textGrob("Month", gp = gpar(cex = 1.3)),
                top = textGrob(paste0(SiteID, " monthly water balance in ", Yr),
                               gp=gpar(fontface="bold", col="black",  fontsize=22)))
ggsave("WaterBalance_Monthly_Horizontal.jpg", width = 15, height = 9, path = FigDir, bg="white")
rm(Hist.WBplot, CF1.WBplot,CF2.WBplot,CF3.WBplot,WBgrid)
...
var_bar_plot(AnnualWB_in, "max_pack.in", cols=colors3, ylab="Max SWE (in)",
             title=paste0(SiteID, " Average annual max SWE in ", Yr, " vs ", BasePeriod))
...
CF3.SWE<-spaghetti_plot_wateryr(subset(WBData,CF %in% CFs[3]),"PACK.in",col=col[4], CF=CFs[3])

SWEgrid <- ggarrange(Hist.SWE, CF1.SWE, CF2.SWE, CF3.SWE, ncol = 1, nrow = 4,common.legend = T)
...
CF3.runoff<-spaghetti_plot_wateryr(subset(WBData,CF %in% CFs[3]),"Runoff.in",col=col[4], CF=CFs[3])

runoffgrid <- ggarrange(Hist.runoff, CF1.runoff, CF2.runoff, CF3.runoff,ncol = 1, nrow = 4,common.legend = T)
...
CF3.AET<-spaghetti_plot_wateryr(subset(WBData,CF %in% CFs[3]),"AET.in",col=col[4], CF=CFs[3])

aetgrid <- ggarrange(Hist.AET, CF1.AET, CF2.AET, CF3.AET,ncol = 1, nrow = 4,common.legend = T)
...
CF3.SM<-spaghetti_plot_wateryr(subset(WBData,CF %in% CFs[3]),"SOIL.in",col=col[4], CF=CFs[3])

SMgrid <- ggarrange(Hist.SM, CF1.SM, CF2.SM, CF3.SM, ncol = 1, nrow = 4,common.legend = T)
