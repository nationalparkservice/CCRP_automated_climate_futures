...
checkFile <- function(fName, fileURL=F){
  ...
  if(fName %in% tmpFiles){
    ...
    if("zip" == tolower(str_sub(fName, str_length(fName)-2, str_length(fName))))unzip(paste(DataDir,fName,sep=''),exdir=substring(DataDir, 1, nchar(DataDir) - 1))
    return(1)
  }  # 1 file exists
...
histInFile <-  str_c(Koppen,"_historical.zip", sep='')        # histInFile <- file.choose()    #  browse to path/file
projInFile <- str_c(Koppen,"_future.zip", sep='')
checkFile(histInFile)

Gridmet<- read.csv(here::here(DataDir, str_c(Koppen,"_historical.csv", sep='')))
...
Future_all <- read.csv(here::here(DataDir, str_c(Koppen,"_future.csv", sep='')))
