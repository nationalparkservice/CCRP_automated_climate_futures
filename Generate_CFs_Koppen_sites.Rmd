...
library(gridExtra); library(SPEI); library(tidyr); library(tibble); library(sp); library(skimr); library(cft); library(stringr); library(ggpubr); library(lemon); library(ggfortify); library(extRemes); library(terra)
...
SiteID = "NIOB"  
Koppen = "NIOB_21" #PARK_##
SiteName = "NIOB-West" #optional - set at SiteID if don't want to use
set.seed(50)
...
OutDir = paste0('./PARK/',Koppen,"/") # for .csv's
...
source(here::here("scripts", "get_site_info_Koppen.R")) # Extract local inputs; seconds to run

source(here::here("scripts", "create_site_maps_Koppen.R")) # Create site maps

source(here::here("scripts", "get_params_Koppen.R"))
```
...
SiteID = Koppen
if(centroids_csv == "Y") {
  source(here::here("scripts", "download_Park_csvs_Koppen.R"), local = knitr::knit_global())
} else {
...
load(paste0(OutDir,"input-data/Final_Environment.RData"))
...
source(here::here("scripts", "copy_paste_delete_files_Koppen.R")) # copies files and folders to specified local folder
